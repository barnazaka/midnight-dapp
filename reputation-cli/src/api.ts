...
import { Reputation, type ReputationPrivateState, witnesses } from '@midnight-ntwrk/reputation-contract';
...
import {
  type ReputationCircuits,
  type ReputationContract,
  type ReputationPrivateStateId,
  type ReputationProviders,
  type DeployedReputationContract,
} from './common-types';
...
export interface ReputationLedgerState {
  publicAverage: bigint;
  reviewCount: bigint;
}

export const getReputationLedgerState = async (
  providers: ReputationProviders,
  contractAddress: ContractAddress,
): Promise<ReputationLedgerState | null> => {
  ...
  const state = await providers.publicDataProvider
    ...
    .then((contractState) => {
        ...
        const l = Reputation.ledger(contractState.data);
        return {
            publicAverage: l.public_average,
...
export const reputationContractInstance: ReputationContract = new Reputation.Contract(witnesses);

export const joinContract = async (
  providers: ReputationProviders,
  contractAddress: string,
): Promise<DeployedReputationContract> => {
  const reputationContract = await findDeployedContract(providers, {
    contractAddress,
  ...
  logger.info(`Joined contract at address: ${reputationContract.deployTxData.public.contractAddress}`);
  return reputationContract;
};

export const deploy = async (
  providers: ReputationProviders,
  privateState: ReputationPrivateState,
): Promise<DeployedReputationContract> => {
  logger.info('Deploying reputation contract...');
  const reputationContract = await deployContract(providers, {
    compiledContract: reputationCompiledContract,
    privateStateId: 'reputationPrivateState',
    initialPrivateState: privateState,
  });
  logger.info(`Deployed contract at address: ${reputationContract.deployTxData.public.contractAddress}`);
  return reputationContract;
};

export const postReview = async (reputationContract: DeployedReputationContract, commitment: string): Promise<FinalizedTxData> => {
  logger.info('Posting review...');
  const finalizedTxData = await reputationContract.callTx.post_review(Uint8Array.from(Buffer.from(commitment, 'hex')));
  logger.info(`Transaction ${finalizedTxData.public.txId} added in block ${finalizedTxData.public.blockHeight}`);
  return finalizedTxData.public;
};

export const aggregateReview = async (reputationContract: DeployedReputationContract, commitment: string): Promise<FinalizedTxData> => {
  logger.info('Aggregating review...');
  const finalizedTxData = await reputationContract.callTx.aggregate_review(Uint8Array.from(Buffer.from(commitment, 'hex')));
  logger.info(`Transaction ${finalizedTxData.public.txId} added in block ${finalizedTxData.public.blockHeight}`);
  return finalizedTxData.public;
};

export const publishReputation = async (reputationContract: DeployedReputationContract): Promise<FinalizedTxData> => {
  logger.info('Publishing reputation...');
  const finalizedTxData = await reputationContract.callTx.publish_reputation();
  logger.info(`Transaction ${finalizedTxData.public.txId} added in block ${finalizedTxData.public.blockHeight}`);
  return finalizedTxData.public;
};

export const displayReputationValue = async (
  providers: ReputationProviders,
  reputationContract: DeployedReputationContract,
): Promise<{ reputationState: ReputationLedgerState | null; contractAddress: string }> => {
  const contractAddress = reputationContract.deployTxData.public.contractAddress;
  const reputationState = await getReputationLedgerState(providers, contractAddress);
  if (reputationState === null) {
    logger.info(`There is no reputation contract deployed at ${contractAddress}.`);
  } else {
    logger.info(`Current reputation: ${Number(reputationState.publicAverage)} (from ${Number(reputationState.reviewCount)} reviews)`);
  }
  return { contractAddress, reputationState };
};
...
export const configureProviders = async (ctx: WalletContext, config: Config) => {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<ReputationCircuits>(contractConfig.zkConfigPath);
  return {
